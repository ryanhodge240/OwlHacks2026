...
function hashSession(token) {
  return crypto.createHmac('sha256', sessionSecret || 'development-session-secret').update(token).digest('hex');
}
...
app.post('/api/auth/login', authLimiter, async (request, response) => {
  ...
  const password = String(request.body.password || '');
  const result = await pool.query(
    'SELECT id, username, password_hash FROM users WHERE username = LOWER($1)',
    [username],
  );
